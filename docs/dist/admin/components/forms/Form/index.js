...
const WatchFormErrors_1 = require("./WatchFormErrors");
const splitPathByArrayFields_1 = require("../../../../utilities/splitPathByArrayFields");
const setsAreEqual_1 = require("../../../../utilities/setsAreEqual");
const buildFieldSchemaMap_1 = require("./buildFieldSchemaMap");
...
const Form = (props) => {
    ...
    const { id, getDocPreferences, collection, global } = (0, DocumentInfo_1.useDocumentInfo)();
    ...
    const buildRowErrors = (0, react_1.useCallback)(() => {
        ...
        Object.entries(fields).forEach(([path, field]) => {
            ...
            for (let i = 0; i < pathSegments.length; i += 1) {
                ...
                const formField = fields === null || fields === void 0 ? void 0 : fields[fieldPath];
                // Is this an array or blocks field?
                if (Array.isArray(formField === null || formField === void 0 ? void 0 : formField.rows)) {
        ...
        Object.entries(newFieldRows).forEach(([path, newRows]) => {
            const stateMatches = newRows.every((newRow, i) => {
                var _a;
                const existingRowErrorPaths = (_a = existingFieldRows[path][i]) === null || _a === void 0 ? void 0 : _a.childErrorPaths;
                return (0, setsAreEqual_1.setsAreEqual)(newRow.childErrorPaths, existingRowErrorPaths);
            });
            if (!stateMatches) {
                dispatchFields({
                    type: 'UPDATE',
    ...
    const traverseRowConfigs = react_1.default.useCallback(({ pathPrefix, path, fieldConfig }) => {
        const config = fieldConfig;
        const pathSegments = (0, splitPathByArrayFields_1.splitPathByArrayFields)(path);
        const configMap = (0, buildFieldSchemaMap_1.buildFieldSchemaMap)(config);
        for (let i = 0; i < pathSegments.length; i += 1) {
            const pathSegment = pathSegments[i];
            if ((0, isNumber_1.isNumber)(pathSegment)) {
    ...
    const addFieldRow = (0, react_1.useCallback)(async ({ path, rowIndex, data }) => {
        ...
        const fieldConfig = getRowConfigByPath({
            path,
            blockType: data === null || data === void 0 ? void 0 : data.blockType,
        });
        if (fieldConfig) {
            const subFieldState = await (0, buildStateFromSchema_1.default)({ fieldSchema: fieldConfig, data, preferences, operation, id, user, locale, t });
            dispatchFields({ type: 'ADD_ROW', rowIndex, path, blockType: data === null || data === void 0 ? void 0 : data.blockType, subFieldState });
        }
    }, [dispatchFields, getDocPreferences, id, user, operation, locale, t, getRowConfigByPath]);
    const removeFieldRow = (0, react_1.useCallback)(async ({ path, rowIndex }) => {
        dispatchFields({ type: 'REMOVE_ROW', rowIndex, path });
    }, [dispatchFields]);
    const replaceFieldRow = (0, react_1.useCallback)(async ({ path, rowIndex, data }) => {
        const preferences = await getDocPreferences();
        const fieldConfig = getRowConfigByPath({
            path,
            blockType: data === null || data === void 0 ? void 0 : data.blockType,
        ...
        if (fieldConfig) {
            const subFieldState = await (0, buildStateFromSchema_1.default)({ fieldSchema: fieldConfig, data, preferences, operation, id, user, locale, t });
            dispatchFields({ type: 'REPLACE_ROW', rowIndex, path, blockType: data === null || data === void 0 ? void 0 : data.blockType, subFieldState });
        }
    }, [dispatchFields, getDocPreferences, id, user, operation, locale, t, getRowConfigByPath]);
                ...
                react_1.default.createElement(context_1.SubmittedContext.Provider, { value: submitted },
                    react_1.default.createElement(context_1.ProcessingContext.Provider, { value: processing },
                        react_1.default.createElement(context_1.ModifiedContext.Provider, { value: modified },
                            react_1.default.createElement(context_1.FormFieldsContext.Provider, { value: fieldsReducer },
                                react_1.default.createElement(WatchFormErrors_1.WatchFormErrors, { buildRowErrors: buildRowErrors }),
                                children))))))));
};
