...
class ParamParser {
    constructor({ req, collectionSlug, globalSlug, versionsFields, model, where, access, overrideAccess, }) {
        this.req = req;
    ...
    async parse() {
        ...
        if (query)
            result.$and.push(query);
        if (typeof this.access === 'object') {
            const accessQuery = await this.parsePathOrRelation(this.access, true);
            if (accessQuery)
                result.$and.push(accessQuery);
        }
        return result;
    }
    async parsePathOrRelation(object, overrideAccess) {
        let result = {};
    ...
    async buildAndOrConditions(conditions, overrideAccess) {
    ...
    async buildSearchParam({ fields, incomingPath, val, operator, overrideAccess, }) {
        var _a;
    ...
    async getLocalizedPaths({ collectionSlug, globalSlug, fields, incomingPath, overrideAccess, }) {
        var _a;
...
const getBuildQueryPlugin = ({ collectionSlug, versionsFields, } = {}) => {
    return function buildQueryPlugin(schema) {
        ...
        async function buildQuery({ req, where, overrideAccess = false, access, globalSlug }) {
