...
async function deleteLocal(payload, options) {
    var _a, _b, _c;
    const { collection: collectionSlug, depth, id, where, locale = null, fallbackLocale = null, user, overrideAccess = true, showHiddenFields, context, } = options;
    ...
    if (!collection) {
        throw new errors_1.APIError(`The collection with slug ${String(collectionSlug)} can't be found. Delete Operation.`);
    }
    const req = {
        user,
        payloadAPI: 'local',
